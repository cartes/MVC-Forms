...
if ($functions) {
    ...
    function get_site_path() {
        return 'http://'.$_SERVER['HTTP_HOST'].'/residentia';
    }
    ...
    function get_header($param = '') {
        global $titulo;
        $ext = '.php';
        $file = APPPATH . '/views/header';
        if($param != '') {
            $final_file = $file .'-' . $param . $ext;
        } else {
            $final_file = $file . $ext;
        }
...
class phpformbuilder {
    ...
    function add_input( $label, $args = '', $slug = '' ) {
            ...
            $this->inputs[ $slug . $this->index ] = $args;
    ...
    function build_form( $echo = true ) {
        ...
        foreach ( $this->inputs as $val ) :
                ...
                if ( $val['type'] === 'range' || $val['type'] === 'number' ) {
                    $min_max_range .= ( $val['min'] != '' ) ? ' min="' . $val['min'] . '"' : '';
                ...
                $attr .= $val['placeholder'] ? ' placeholder="' . $val['placeholder'] .'"' : '';
                $attr .= $val['style'] ? ' style="' . $val['style'] .'"' : '';
                // Build the label
                if ( ! empty( $label_html ) ) {
                    $field .= $label_html;
